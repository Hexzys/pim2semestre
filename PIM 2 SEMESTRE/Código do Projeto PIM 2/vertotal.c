...
int vertotal(){ //função que mostra o total dos relatorios
...
FILE *pont_arq10; //criar ponteiro para arquivo txt
    pont_arq10 = fopen("..\\Sistema de Clinicas UNIP\\relatorios\\total_dia_mes-sul.txt" ,"w");

FILE *pont_arq11; //criar ponteiro para arquivo txt
    pont_arq11 = fopen("..\\Sistema de Clinicas UNIP\\relatorios\\total_dia_mes-leste.txt" ,"w");

FILE *pont_arq12; //criar ponteiro para arquivo txt
    pont_arq12 = fopen("..\\Sistema de Clinicas UNIP\\relatorios\\total_dia_mes-oeste.txt" ,"w");
...
if (strcmp(rede, "sul") == 0){
  DIR *dir;
   ...
   //Mas quando o usuário digita -1 é finalizado
    setcolor(244);
    printf("\n ATENÇAO: Digite apenas numeros inteiros (sem ,00)");
...
printf("\n\n Digite 1 para o relatório diário/mensal de toda rede, \n 2 para voltar ao Menu e/ou 0 para encerrar sessão:");
scanf("%d",&menu);
if (menu == 1){
    vertotalrede();
}
